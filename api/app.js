...
//   ?action=preview  -> compute calendar, NEVER writes (used live as targets edit)
//   ?action=state    -> GET settings (open) / POST settings (needs x-app-password)
//   ?action=run      -> daily cron + manual sync; writes to OwnerRez ONLY if auto_sync ON
// Auth: state POST + run need x-app-password=APP_PASSWORD (run also accepts Bearer CRON_SECRET).
...
const FLOOR=99, CEIL=300, ENDPOINT="https://api.ownerrez.com/v2/spotrates";
const UNITS=[
...
const DEFAULTS={targets:SEED_TARGETS,knobs:KNOBS,auto_sync:false,learned:{raiseFactor:1}};
const SKEY="parkside:state";

async function getState(){ if(!redis) return {...DEFAULTS}; const s=await redis.get(SKEY); return {...DEFAULTS,...(s||{})}; }
async function setState(p){ const cur=await getState(); const next={...cur,...p}; if(redis) await redis.set(SKEY,next); return next; }

function paceFraction(l){ if(l<=0)return 1; if(l>=365)return .05; return Math.max(.05,Math.min(1,Math.exp(-l/110))); }
function targetFor(d,t){ const m=t[d.getUTCMonth()+1]; return KNOBS.weekendDays.includes(d.getUTCDay())?m.we:m.wd; }
function controllerMult(tg,occ,lead,learned){ const exp=tg*paceFraction(lead); const gap=occ-exp; const rf=(learned&&learned.raiseFactor)||1;
...
async function getOccupancy(start,days){
  const user=process.env.OWNERREZ_API_USER, token=process.env.OWNERREZ_API_TOKEN; if(!user||!token) return ()=>0;
  const auth="Basic "+Buffer.from(`${user}:${token}`).toString("base64"); const W=21,WE=[5,6];
  const s=new Date(start+"T00:00:00Z"); const booked={};
  try{ for(const u of UNITS){
    ...
    for(const b of items){ const ci=new Date(((b.arrival||b.checkIn||b.arrival_date||"")+"").slice(0,10)+"T00:00:00Z"), co=new Date(((b.departure||b.checkOut||b.departure_date||"")+"").slice(0,10)+"T00:00:00Z");
      if(isNaN(ci)||isNaN(co))continue; for(let d=new Date(ci);d<co;d.setUTCDate(d.getUTCDate()+1)) booked[u.orp+"|"+d.toISOString().slice(0,10)]=true; }
  } }catch{ return ()=>0; }
  return (ds,we)=>{ const c=new Date(ds+"T00:00:00Z"); let b=0,t=0; for(let o=-W;o<=W;o++){ const d=new Date(c); d.setUTCDate(d.getUTCDate()+o);
...
function compute(signalMap,targets,learned,today,startDate,days,occFor){
  const start=new Date(startDate+"T00:00:00Z"); const out=[];
  for(let i=0;i<days;i++){ const d=new Date(start); d.setUTCDate(d.getUTCDate()+i); const ds=d.toISOString().slice(0,10);
    const we=KNOBS.weekendDays.includes(d.getUTCDay()); const tg=targetFor(d,targets); const occ=occFor(ds,we);
    let sig=signalMap[ds]; if(sig==null) sig=signalFallback(signalMap,ds);
    const lead=Math.round((Date.parse(ds)-Date.parse(today))/86400000);
    for(const u of UNITS){ const raw=(sig+u.offset)*controllerMult(tg,occ,lead,learned); out.push({property_id:u.orp,unit:u.name,date:ds,amount:clampRound(raw),currency:"USD"}); }
  } return out;
}
function validate(es){ const ok=[]; for(const e of es){ const pid=Number(e.property_id), amt=Number(e.amount);
  if(Number.isInteger(pid)&&/^\d{4}-\d{2}-\d{2}$/.test(e.date)&&amt>=FLOOR&&amt<=CEIL&&e.currency==="USD") ok.push({property_id:pid,date:e.date,amount:Math.round(amt),currency:"USD"}); } return ok; }
...
module.exports=async(req,res)=>{
  try{
    ...
    if(action==="state"){
      if(req.method==="GET"){ const s=await getState(); return res.status(200).json({targets:s.targets,knobs:s.knobs,auto_sync:s.auto_sync}); }
      if(req.method==="POST"){ if((req.headers["x-app-password"]||"")!==(process.env.APP_PASSWORD||"")) return res.status(401).json({error:"unauthorized"});
        ...
        const p={}; if(b&&b.targets)p.targets=b.targets; if(b&&typeof b.auto_sync==="boolean")p.auto_sync=b.auto_sync;
        const n=await setState(p); return res.status(200).json({ok:true,auto_sync:n.auto_sync}); }
    ...
    if(action==="preview"){
      ...
      const sig=await getSignal(); const occ=await getOccupancy(today,days);
      const rates=compute(sig,targets,st.learned,today,today,days,occ);
      return res.status(200).json({mode:"PREVIEW",wrote:false,count:rates.length,min:Math.min(...rates.map(r=>r.amount)),max:Math.max(...rates.map(r=>r.amount)),rates});
    }
    if(action==="run"){
      ...
      const sig=await getSignal(); const occ=await getOccupancy(today,days);
      const rates=compute(sig,st.targets,st.learned,today,today,days,occ);
      ...
      const r=await pushOwnerRez(rates); return res.status(r.ownerrezOk?200:502).json({mode:"LIVE_SYNC",auto_sync:true,...r});
    }
