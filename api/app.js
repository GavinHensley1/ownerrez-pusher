...
//  preview (no write) | state (GET/POST+pw) | run (cron/manual; pushes only if auto_sync ON)
//  occupancy (per-unit/month + booked dates, from iCal) | logs (Phase-1 learning data, pw)
// Occupancy source = the channels' read-only iCal calendars, fetched server-side (import only,
// nothing pushed out). Phase-1 logging records ONLY this project's own forward booking outcomes.
...
const DEFAULTS={targets:SEED_TARGETS,knobs:KNOBS,auto_sync:false,learned:{raiseFactor:1},overrides:{},icals:{}};
...
// Revenue-max held to target, compared DIRECTLY; cold-start (no occupancy data) -> neutral (market).
...
// ---- iCal occupancy (read-only import; nothing pushed out) ----
...
function occInfoFrom(booked){ const WE=[5,6],W=21;
  const fn=(ds,we)=>{ const c=new Date(ds+"T00:00:00Z"); let b=0,t=0; for(let o=-W;o<=W;o++){const d=new Date(c);d.setUTCDate(d.getUTCDate()+o); if(WE.includes(d.getUTCDay())!==we)continue; const k=d.toISOString().slice(0,10); for(const u of UNITS){t++; if(booked.byUnit[u.orp][k])b++;}} return t?b/t:0; };
  return {fn,total:booked.total}; }
...
// ---- Phase 1 learning log: forward-looking, own outcomes only (no historical/market-comp) ----
  ...
  try{
    ...
    if(action==="state"){
      if(req.method==="GET"){ const s=await getState(); const icalCount={}; for(const u of UNITS) icalCount[u.orp]=(s.icals[u.orp]||[]).length;
        return res.status(200).json({targets:s.targets,knobs:s.knobs,auto_sync:s.auto_sync,overrides:s.overrides||{},icalCount}); }
      if(req.method==="POST"){ if((req.headers["x-app-password"]||"")!==(process.env.APP_PASSWORD||"")) return res.status(401).json({error:"unauthorized"});
        ...
        if(b&&b.icals)p.icals=b.icals;                 // {orp:[url,...]}
        if(b&&b.icalSet){ const ic={...(cur.icals||{})}; ic[b.icalSet.orp]=b.icalSet.urls; p.icals=ic; }
        ...
        const n=await setState(p); if(redis&&(b.icals||b.icalSet)) await redis.del("parkside:booked");
    ...
    if(action==="preview"){
      ...
      const occInfo=(b&&typeof b.mockOcc==="number")?{fn:()=>b.mockOcc,total:1}:occInfoFrom(await getBooked(st,today,days));
      const rates=compute(sig,targets,st.learned,today,today,days,occInfo,st.overrides); const amts=rates.map(r=>r.amount);
    ...
    if(action==="run"){
      ...
      const rates=compute(sig,st.targets,st.learned,today,today,days,occInfoFrom(booked),st.overrides);
      const logged=await logPhase1(rates,booked,today);   // Phase-1 logging runs daily regardless of auto_sync
